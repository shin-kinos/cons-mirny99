...
pub fn seq_weight( seq_list : &Vec<String>, site_list : &Vec<String>, arg_m : &String, arg_t : &String ) -> Vec<f64>
{
	/*
	 * Amino acid list for Position-Based mothod.
	 * It's static variable as it's also used for calculating Shannon's entropy.
	*/
	unsafe {
	...
	//println!( "pub fn seq_weight(), &seq_list  : {:p}", seq_list );
	//println!( "pub fn seq_weight(), &site_list : {:p}", site_list );

	//let num_seq  : usize = ( *seq_list ).len();

	//let mut weight_list : Vec<f64> = vec![ 1.0; num_seq ];

	if *arg_m == "va" { /* weight_list = */ weight_va( seq_list ) }
	else              { /* weight_list = */ weight_henikoff( site_list /*, arg_t */ ) }
...
{
	//println!( "fn weight_henikoff(), &site_list  : {:p}", site_list );
...
{
	//println!( "fn weight_va(), &seq_list  : {:p}", seq_list );
	...
	*/
	...
	println!( "\nSum of sequence weighting : {:.4}", sum_norm_weight );
